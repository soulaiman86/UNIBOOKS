...
function SellerOverview() {
    ...
    useEffect(() => {
        const fetchStats = async () => {
            try {
                const res = await axios.get(`http://localhost:5000/api/seller/seller-stats/${sellerId}`);
                setStats(res.data);
            } catch (err) {
    ...
    return (
        <div>
            <h2 style={{ marginBottom: '25px', color: '#2c3e50' }}>📊 My Overview</h2>
...
const cardsContainerStyle = { display: 'flex', flexWrap: 'wrap', gap: '20px' };
const cardStyle = (color) => ({ backgroundColor: color, color: 'white', padding: '20px 25px', borderRadius: '10px', minWidth: '150px', textAlign: 'center', boxShadow: '0 4px 10px rgba(0,0,0,0.1)' });
const cardNumberStyle = { margin: 0, fontSize: '2rem', fontWeight: 'bold' };
const cardLabelStyle = { margin: '5px 0 0 0', fontSize: '0.9rem', opacity: 0.9 };
